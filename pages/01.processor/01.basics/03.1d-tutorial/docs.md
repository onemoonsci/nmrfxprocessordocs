...
I> NMRFx lets you look at the "indirect FID".  It does this by assembling the datapoints in the FID file that correspond to the first time increment in the direct dimension.  At present this only works properly for 2D datasets, but development of the code for higher dimensions is in progress.  The ability to observe signal in the indirect FID also depends on the type type of experiment and signal-to-noise ratio.  The technique works well for the 2D NOESY used here.
