...
Generating multiple structures with the **batch** subcommand will generate a variety of output files.  These are described [here](#batchoutputlink).
...
### <a id="batchoutputlink">Multiple Structure Calculation (batch subcommand) Output</a>
...
The [summary](#summarylink) subcommand will be used to generate an analysis.txt file in
the final directory.  It will contain distance violations and close contacts that have distance violations
greater than a specified amount (default 0.2) and are present in 
more than a specified (default 2) structures:

```
  Type            Atom1              Atom2 nViol      Bound       Mean        Max Structures
   Dis:          1:50.HA -        1:50.HD21    3       2.55       0.12       0.45 |..++..+...|
   Dis:          1:50.HA -        1:50.HD22    3       2.55       0.12       0.45 |..++..+...|
   Dis:          1:50.HA -        1:50.HD23    3       2.55       0.12       0.45 |..++..+...|
```

A summary of energy terms from the last line of the violation files (final1.txt, final2.txt etc.) will be
